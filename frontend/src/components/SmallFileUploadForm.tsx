...
// Maximum file size (matching backend) - 10MB in bytes
...
const SmallFileUploadForm = () => {
  ...
  const validateFileSize = (file: File): boolean => {
    if (file.size > MAX_FILE_SIZE) {
      ...
      setDetailedError(
        `The selected file exceeds the maximum size of ${MAX_FILE_SIZE_FORMATTED}`
      );
      return false;
    }
    return true;
  ...
  // Handle form submission - connect to the backend API
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try {
          ...
          // Set primary error message (short)
          setErrorMessage(response.error || 'Upload failed');

          // Set detailed message if available
          setDetailedError(response.message || null);
    ...
    xhr.onerror = () => {
      ...
      setDetailedError('Could not connect to the server. Is it running?');
    };
